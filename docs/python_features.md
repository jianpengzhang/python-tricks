...
Python默认定义了三代对象集合，索引数越大，对象存活时间越长。根据“分代假设”（generational hypothesis），大多数对象通常会在创建后的短时间内变得不可达，因此新创建的对象会放在第 0 代，回收频率较高；而那些长时间存活的对象会进入第 1 代和第 2 代，回收频率较低。分代策略有效减少了不必要的回收操作，提高了垃圾回收的性能。
...
在 Python 中，`is`, `is not`, `==`, `!=` 都是用于比较的运算符，但它们的作用和含义是不同的：  

* `is`, `is not` 检查的是两个对象是否是同一个对象，即是否指向内存中的同一块地址    
  * `is`：用于判断两个对象是否是同一个对象（即它们的内存地址是否相同）；
  * `is not`：用于判断两个对象是否不是同一个对象；
  * 示例：  
    ```python
    a = [1, 2, 3]
    b = a  # b 引用与 a 相同的对象
    c = [1, 2, 3]  # c 是一个新的对象，但内容相同
    
    print(a is b)  # True，a 和 b 是同一个对象
    print(a is c)  # False，a 和 c 是不同的对象，尽管它们内容相同
...
* `==`,`!=` 对比的是两个对象的值  
  ...
  * 示例：  
    ...
    print(a == b)  # True，a 和 b 内容相同，因为 b 引用的是 a
    print(a == c)  # True，a 和 c 内容相同，尽管它们是不同的对象
    print(a != c)  # False，a 和 c 内容相同
    ```
* 注意  

  对于不可变类型（如 str, tuple 等），因为它们的值不能改变，Python 会对其进行”缓存“或”内部优化“，在很多情况下，Python 会使多个变量引用相同的内存位置（即同一个对象），这时 `is` 和 `==` 会返回相同的结果。但请注意，这并不意味着 `is` 和 `==` 总是等价，尤其是当不可变对象的值非常大或复杂时，Python 可能会选择创建新的对象，而不是复用旧对象。因此，`==` 比较的是对象的值，而 `is` 比较的是对象的地址。  
  ...
  print(a is b)  # True，a 和 b 可能指向同一内存地址（Python 对短字符串进行优化）
  print(a is c)  # True，a 和 c 引用相同的对象
  
  # 测试大字符串
  d = "a" * 10000  # 创建一个大的字符串
  e = "a" * 10000
  print(d == e)  # True，内容相同
  print(d is e)  # False，可能不指向相同的内存地址
  ```
  在不可变类型中，`is` 和 `==` 在很多情况下是等价的，但不能完全保证在所有情况下都等价。

* 结论  
  * 对于不可变类型（如 str, tuple 等），`is` 和 `==` 在很多情况下可能等价，但不能完全保证，在一些复杂情况下，`is` 和 `==` 会有所不同；
  * 对于可变类型（如 list, dict 等），`is` 和 `==` 是有区别的，`==` 比较的是对象的内容，而 `is` 比较的是对象的内存地址；

## 27. read,readline和readlines

在 Python 中，`read`, `readline` 和 `readlines` 是用于从文件中读取内容的三种常用方法。它们有不同的行为和用途，下面是对每种方法的详细介绍和示例。

### 27.1. **`read()`**
`read()` 方法读取文件的全部内容，并将其作为一个字符串返回。如果文件非常大，可以通过给 `read()` 方法传递一个参数来指定读取的字符数。
...
- **`size`**：指定要读取的字符数。如果不指定或指定为 `-1`，则读取整个文件。
- 返回：一个字符串，包含文件的内容。

#### 27.1.2 示例
```python
# 假设文件 "example.txt" 内容为：
# Hello, World!
# This is a test file.
...
#### 27.1.3 用法
- 适用于读取整个文件或一次读取文件的全部内容。
- 适用于文件较小或在内存充足的情况下读取整个文件。

### 27.2. **`readline()`**
`readline()` 方法读取文件中的一行，并将该行作为字符串返回。每次调用 `readline()` 时，文件指针会向下移动到下一行。
...
- **`size`**：指定要读取的字符数。如果未指定或指定为 `-1`，则读取整行，包括换行符（`\n`）。如果指定了 `size`，则会读取指定字符数，可能只读取部分行。
- 返回：当前行的内容（字符串），包括行末的换行符（`\n`）。
...
with open('example.txt', 'r') as file:
    ...
    print(line2)  # 输出：This is a test file.
```

#### 27.2.3 用法
- 适用于逐行读取文件。可以多次调用 `readline()` 来逐行读取文件的内容，直到文件结束。
- `readline()` 适合处理逐行读取和按行操作的场景，尤其在需要逐行处理时非常有用。

### 27.3. **`readlines()`**
`readlines()` 方法读取文件中的所有行，并将它们作为列表返回。每一行作为列表中的一个元素。每个元素末尾通常包含一个换行符（`\n`）。
...
- **`hint`**：指定要读取的字节数。如果未指定或指定为 `-1`，则读取整个文件。可以限制读取的大小。
- 返回：一个列表，包含文件的每一行。
...
#### 27.3.3 用法
- 适用于读取整个文件并按行分割成一个列表，特别适用于需要按行访问内容的场景。
- 适合文件不是非常大的情况下，方便后续对每行内容进行处理。

### 27.4. 主要区别
| 方法         | 返回类型       | 读取方式               | 是否包含换行符 |
|--------------|----------------|------------------------|---------------|
| `read()`     | 字符串         | 一次性读取整个文件内容 | 含换行符       |
| `readline()` | 字符串         | 逐行读取文件           | 含换行符       |
| `readlines()`| 列表           | 逐行读取文件并返回列表 | 每个元素末尾含换行符 |

### 27.5. 小结
- **`read()`**：用于一次性读取整个文件的内容，适用于小文件。
- **`readline()`**：逐行读取，适用于需要按行处理文件的情况。
- **`readlines()`**：将文件的所有行读取到一个列表中，适用于需要操作文件所有行的场景。
...
#### 28.1.1 单向链表（Singly Linked List）
...
#### 28.1.2 双向链表 (Doubly Linked List)
...
#### 28.1.3 循环链表 (Circular Linked List)
...
扩展阅读：  
(1) [Python List](https://www.jianshu.com/p/J4U6rR)