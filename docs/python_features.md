...
Python 中常用的设计模式包括以下几种：  
* 单例模式：单例模式确保一个类只有一个实例，并提供一个全局访问点。它适用于需要唯一对象的场景，如：数据库连接池（管理与数据库的唯一连接）、配置管理器（读取和管理系统配置）等；
* 工厂模式：工厂模式提供创建对象的接口，而不指定具体类。它解耦了对象的创建和使用。如：日志系统（根据配置生成不同类型的日志对象）等；
* 观察者模式：观察者模式定义了对象间的一对多依赖，当一个对象状态改变时，所有依赖者都会收到通知并自动更新，其主要通过“发布-订阅”模型实现，发布者管理观察者列表，当状态改变时，通知观察者。如：消息队列（发布者发布消息，多个消费者处理消息）等；
* 装饰器模式：装饰器模式允许动态添加行为到对象上，且不改变其结构。它在功能扩展时比继承更灵活。如：日志记录（为函数添加日志记录功能）、权限检查（在执行操作前检查权限）等；
* 策略模式：策略模式定义了算法族，并使它们互换使用，这使得算法在客户端独立变化。如：支付系统（支持多种支付方式，用户选择具体支付策略）、数据压缩（支持不同的压缩算法，基于文件类型选择策略）等；
* 适配器模式：适配器模式用于将一个接口转换为客户期望的另一接口；
